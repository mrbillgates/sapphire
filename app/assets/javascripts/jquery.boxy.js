...
jQuery.extend(Boxy, {
    ...
    ask: function(question, answers, callback, options) {
        ...
        var buttons = jQuery('<form accept-charset="UTF-8" action="/delete" id="form_delete" method="get"></form>');
        buttons.html(jQuery.map(Boxy._values(answers), function(v) {
            return (v != options.cancel) ? "<input type='button' value='" + v + "' />" : "<input type='button' value='" + v + "' />";

        }).join(' '));

        buttons.append('<input type="text" style="display: none" name="id" value="' + options.userid + '">');
